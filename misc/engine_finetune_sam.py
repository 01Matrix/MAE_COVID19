...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    args=None):
    model.train(True)
    metric_logger = misc_sam.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', misc_sam.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # loss1 = criterion(model(samples), targets)
        # loss1.backward()
        loss1 = optimizer.step(closure)
        # optimizer.zero_grad()

        # loss1 = criterion(model(samples), targets)
        # loss_scaler(loss1, optimizer, closure=torch.enable_grad(), clip_grad=max_norm,
        ...
        # criterion(model(samples),targets).backward()
        # optimizer.second_step(zero_grad=True)

        # loss2 = loss2 / accum_iter
        # loss_scaler(loss2, optimizer, closure=torch.enable_grad(), clip_grad=max_norm,
        #             parameters=model.parameters(), create_graph=False,
        #             update_grad=(data_iter_step + 1) % accum_iter == 0)
        # if (data_iter_step + 1) % accum_iter == 0:
        #     optimizer.second_step(zero_grad=True)
        ...
        loss_value_reduce = misc_sam.all_reduce_mean(loss_value)
