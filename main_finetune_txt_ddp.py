...
def layerwise_ft(flag_attn,flag_mlp,model):
    ...
    if not flag_attn and not flag_mlp: #接近于linear probe,但是和LP不同的是fc_norm层是unfreeze的
        ...
        for n, p in model.named_parameters():
            ...
            if 'fc_norm' in n:
                p.requires_grad = True
    re_init_w(flag_attn,flag_mlp,model)

# Weight re_init 权重重新初始化
def re_init_w(flag_attn,flag_mlp,model):
    if args.block_list != '':
...
def main(args):
    ...
    if args.finetune and not args.fft:
        ...
        layerwise_ft(args.attn,args.mlp,model_without_ddp)
