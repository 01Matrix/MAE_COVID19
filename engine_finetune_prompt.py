...
def train_one_epoch(model: torch.nn.Module, prompter: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        prompted_samples = prompter(samples)
        ...
        with torch.cuda.amp.autocast():
            ...
            outputs = model(prompted_samples)
        ...
        loss_scaler(loss, optimizer, clip_grad=max_norm,
                    parameters=prompter.parameters(), create_graph=False,
...
@torch.no_grad()
def evaluate(phase, data_loader, model, prompter, device):
